...
const NavButton = ({ emoji, label, active = false, onClick, className }:NavButtonProps) => {
    ...
    return(
        <div className='flex flex-col items-center text-center w-16 gap-1 min-w-0'>
            <button type='button' onClick={onClick} className={`w-12 h-12 px-4 py-3 flex flex-col transition-colors items-center outline-none rounded-xl ${bgColor} ${className}`}>
                <span>{emoji}</span>
            </button>
    ...
    return(
      <nav className="w-full bg-white bg-opacity-80 pb-6 pt-4">
            <div className="mx-auto flex max-w-md items-center justify-between px-6 gap-8">
                ...
                <NavButton label='追加' onClick={()=> navigate("/add")} active={true} emoji='+' className='text-white'></NavButton>
                <NavButton label='カレンダー' onClick={()=> navigate("/calendar")} active={location.pathname === "/calendar"} emoji='📅' className='text-white'></NavButton>
                <NavButton label='グラフ' onClick={()=> navigate("/graph")} active={location.pathname === "/graph"} emoji='📊' className='text-white'></NavButton>
            </div>
      </nav>
    )
}
